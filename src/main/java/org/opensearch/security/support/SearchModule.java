...
import org.opensearch.index.query.functionscore.*;
...
import org.opensearch.search.aggregations.bucket.range.*;
...
import org.opensearch.search.aggregations.bucket.terms.heuristic.*;
...
import org.opensearch.search.fetch.subphase.highlight.*;
...
public class SearchModule {
    ...
    private void registerPipelineAggregation(PipelineAggregationSpec spec) {
        ...
        for (Map.Entry<String, Writeable.Reader<? extends InternalAggregation>> resultReader : spec.getResultReaders().entrySet()) {
            namedWriteables.add(
                    new Entry(InternalAggregation.class, resultReader.getKey(), resultReader.getValue())
            );
        }
    }
    ...
    private void registerRescorer(RescorerSpec<?> spec) {
        ...
        namedWriteables.add(new Entry(RescorerBuilder.class, spec.getName().getPreferredName(), spec.getReader()));
    }
    ...
    private void registerSuggester(SuggesterSpec<?> suggester) {
        namedWriteables.add(
                new Entry(SuggestionBuilder.class, suggester.getName().getPreferredName(), suggester.getReader())
        );
    ...
    private <T extends SignificanceHeuristic> void registerSignificanceHeuristic(SignificanceHeuristicSpec<?> spec) {
        ...
        namedWriteables.add(
                new Entry(SignificanceHeuristic.class, spec.getName().getPreferredName(), spec.getReader())
        );
    }
    ...
    public static List<Entry> getIntervalsSourceProviderNamedWritables() {
    ...
    private void registerSort(SortSpec<?> spec) {
        namedWriteables.add(new Entry(SortBuilder.class, spec.getName().getPreferredName(), spec.getReader()));
        namedXContents.add(
                new NamedXContentRegistry.Entry(
                        SortBuilder.class,
                        spec.getName(),
